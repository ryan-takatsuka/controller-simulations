...
t, states, est_states, sensor_time, measurements, u = controller_design.simulate_kalman(sys.A, sys.B, sys.C, sys.D, K, dt, time_vector, r_var, q_var, x0=x0)
...
plt.step(sensor_time, np.rad2deg(est_states[:,0]), label=('Measured states'), c='r')
plt.legend()
plt.grid()
plt.xlabel('Time [sec]')
